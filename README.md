...
- **Emails**: Nodemailer notifications for inquiries, approvals, and rejections (fails gracefully if SMTP isn't configured). Registration has no email verification step.
...
If SMTP variables are left blank, the app logs a skipped-email message instead of failing — inquiries and admin actions all continue to work without email delivery. Registration itself never sends an email.
